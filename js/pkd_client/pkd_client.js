...
class PKDClient {
    ...
    initPgp() {
        this.pgp = openpgp;
        this.pgp.initWorker({path: 'bower_components/openpgp/dist/openpgp.worker.js'});
    }
    ...
    /**
     * Performs fetching data from the server using fetch API. Supplied config may specify any options of the originl
     ...
     *     prepended.
     * @param {Object} config `body` parameter undergoes transformation, it is converted to string using JSON.stringify
    ...
    /**
     * Accepts array of emails to log keys for. Returns Promise that resolves to the hash of {email:{...}}
     * @param {Array} emails
     * @returns {Promise}
     */
    getPublicKeys(emails, nokeys) {
        ...
        return this.fetch(this.conf.api.getPublicKeys, {
            ...
            for (let key of response.data) {
                key.public_key = key.public_key==true || key.public_key==false ? key.public_key : atob(key.public_key); //base64 decode
    ...
    /**
     * Returns promise that resolves to the private key of the requested user keypair
     *
    ...
    decrypt(params) {
        ...
        return this.getUserKeys(params.getKeyHandler, params.setKeyHandler)
            .then(function (keys) {
                ...
                // Decrypt key if password is set
                if (pkd.password)
                    privateKey.decrypt(pkd.password);
     ...
     *
     * @param {Object} params Params for keypair import. See the https://pkd.stage.easycrypt.co/docs/#api-PKD-Post_patchKeyImport
     * @returns {Promise}
     */
    importKeys(params) {
        var self = this;
        return self.fetch(self.conf.api.addKeyPair, {body: params});
